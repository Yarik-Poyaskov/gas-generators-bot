...
export default function Sidebar({ isCollapsed, onToggle }: SidebarProps) {
  ...
  return (
    <aside className={cn(
      "fixed left-0 top-0 z-40 h-screen bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 transition-all duration-300 ease-in-out shadow-xl shadow-slate-200/50 dark:shadow-none",
      ...
      <div className="flex h-full flex-col relative">
        ...
        <button 
          onClick={onToggle}
          className="absolute -right-3 top-10 z-50 h-6 w-6 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center shadow-md hover:text-[#004899] transition-colors"
        >
