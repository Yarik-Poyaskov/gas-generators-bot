...
async def send_trader_status_report_tomorrow(bot: Bot):
    """Генерирует текстовый отчет о статусе поданных графиков на завтра."""
    ...
    if not submitted_schedules:
        print(f"ℹ️ На завтра ({tomorrow_display_str}) ще не подано жодного графіка.")
        return
    ...
    # Отправляем в TEST_SPECIAL_GROUP_ID (временно по просьбе пользователя)
    ...
    if chat_id:
        try:
            ...
            print(f"🚀 Отчет по графикам на завтра ({tomorrow_str}) отправлен в ТЕСТОВУЮ группу.")
        except Exception as e:
            print(f"Помилка відправки звіту: {e}")
    else:
        print("❌ TEST_SPECIAL_GROUP_ID не настроен в .env")

if __name__ == "__main__":
    async def test_run():
        bot = Bot(token=config.bot_token.get_secret_value())
        await run_trader_report_tomorrow(bot)
